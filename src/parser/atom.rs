    ...
    for child in node.children.borrow().iter() {
        match child.data {
            NodeData::Element {
                ...
                match tag_name {
                    "id" => feed.id = text(child.clone()).unwrap_or_else(uuid_gen),
                    ...
                    "link" => {
                        ...
                        let rel = attr("rel", attributes).unwrap_or_else(|| "".to_string());
    ...
    for child in node.children.borrow().iter() {
        match child.data {
            NodeData::Element {
                ...
                let tag_name = name.local.as_ref();
                match tag_name {
                    "id" => entry.id = text(child.clone()).unwrap_or_else(uuid_gen),
                    ...
                    "content" => {
                        ...
                        let attributes = &attrs.borrow();
                        let content_type =
                            attr("type", attributes).unwrap_or_else(|| "text".to_string());
                    ...
                    "link" => {
                        let attributes = &attrs.borrow();
                        let rel =
                            attr("rel", attributes).unwrap_or_else(|| "alternate".to_string());
                        let mime_type =
                            attr("type", attributes).unwrap_or_else(|| "text/html".to_string());
                        let length = attr("length", attributes)
